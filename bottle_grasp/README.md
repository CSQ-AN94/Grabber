...
避障架构本身**。目前唯一在真机上跑通过的场景是 `table_demo`：桌面单瓶，
抓起来验证后放回原位。货架多格位、真实出货（送到取货口）、按商品视觉
识别这几块 2026-07-20 已经写了代码，但**一次都没有现场测量或真机验证过**，
详见下面"现状"一节，不要跳过直接当成能用。
...
### 2. 桌面 demo（`table_demo`，已真机验证过的场景）
...
可覆盖的环境变量（不改变流程拓扑，只改连接/路径/profile）：
...
`output_joints_deg`。全部填完、且 `enabled: true` 之后，先 plan-only 验证：
...
（真正的 `--plan-only` 低速验证目前要手动 SSH 到机器人直接跑
`scripts/bottle_grasp_demo.py --task-mode from-start --plan-only --dispense ...`，
`run_bottle_grasp.sh` 目前只封装了 `--execute` 这一条路径——上真机前自己
按 runbook 先确认过规划链路。）
...
| 场景 | 状态 |
|---|---|
| `table_demo`（桌面单瓶，放回原位） | 2026-07-19真机3次成功抓放（无避障入口），有避障的完整版仍在收尾问题排查中，见 [bottle_grasp_status.md](../docs/handoff/bottle_grasp_status.md) |
...
bottle_grasp/
  ...
  task.py               两个入口(from-observation/from-start) + DeliverMode(放回/出货)
...
test/bottle_grasp/   262个测试，纯逻辑+mock，不连真机/ROS
```
