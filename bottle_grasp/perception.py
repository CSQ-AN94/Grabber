...
from .core import DemoParams, Detection, SafetyAbort
...
def robust_near_cluster(values: np.ndarray, params: DemoParams) -> tuple[float, float]:
    ...
    if mad > params.max_depth_mad_m:
        raise SafetyAbort(f"深度分布过散: MAD={mad * 1000:.1f} mm")
    return median, mad
...
def depth_point_for_detection(
    depth: np.ndarray, det: Detection, K: np.ndarray, params: DemoParams
    ...
    if not all_values:
        raise SafetyAbort("瓶盖/标签/瓶身区域均无有效深度")
