...
| `scripts/bottle_grasp_demo.py` + 4个launcher | 抓水瓶demo（本次主要工作） | Mac一键/机器人上 |
...
代码在 `bottle_grasp/`，入口是 `scripts/bottle_grasp_demo.py`，配套4个一键
launcher脚本（Mac上跑，自动同步代码到机器人+SSH远程执行）。**完整背景、架构、
...
### `scripts/run_bottle_full_cycle.sh` — 完整循环（示教走廊转移）
```bash
scripts/run_bottle_full_cycle.sh record   # 一次性录『垂下→观察位』走廊
scripts/run_bottle_full_cycle.sh plan     # 离线复核，不动机器人
scripts/run_bottle_full_cycle.sh cycle    # 真机：垂下→抓取→放回→垂回
scripts/run_bottle_full_cycle.sh selftest # 判定MoveIt碰撞检查是否修好
```
转移段（垂下↔观察位）走人工示教录制的走廊，抓取段自主视觉。

### `scripts/run_bottle_grasp_autonomous.sh` — 全自主观察位规划
```bash
...
scripts/run_bottle_grasp_autonomous.sh cycle    # 抓取+抬升+放回+退开
```
头部定位→**MoveIt自主规划**（不用示教）到观察位→腕部精定位→抓取。适合目标
几何简单的场景（比如瓶子放桌角、周围空旷）。
...
跟上面三个脚本比，这个是最原始的"从头开始、无参数定制"版本，直接调用
`bottle_grasp_demo.py` 不带 `--resume-at-wrist`/`--full-cycle`/
`--autonomous-observation`，默认会用 profile 里配置的示教走廊做观察位转移。

### `scripts/record_right_arm_guided_path.py` — 录制示教走廊（只读）
```bash
python3 scripts/record_right_arm_guided_path.py --output <path>.json
```
拖动示教时持续只读记录关节角，Ctrl+C停止保存。`run_bottle_full_cycle.sh record`
内部调用的就是这个。
...
```
`run_bottle_full_cycle.sh selftest` 会自动跑这个。用一个罩住整臂的巨型盒子
探针，几秒内判定MoveIt的碰撞检查到底工不工作（当前诊断为不工作，详见
...
python3 scripts/bottle_grasp_demo.py \
  ...
  --full-cycle                     # 完整循环：垂下→观察→抓→放回→垂回
  --autonomous-observation         # 观察位转移强制走MoveIt自主规划，忽略示教走廊
  --place-back                     # 抓取抬升后放回桌面并退开
  --guided-path <path>.json        # 覆盖profile里配置的示教走廊
