...
修复（`demo._grasp_precheck_ok`）：选观察位时就用 candidate_path() 完全
相同的几何（观察姿态朝向=抓取朝向、同一组 roll、同样的围栏+IK+奇异检查）
从候选关节角出发**离线预演抓取接近段**，预演用 10° 软限位余量
（`observation_grasp_margin_deg`，给头部→腕部定位间约 3cm 目标漂移留
空间），任何 roll 都不可行的观察位直接淘汰；全部淘汰则明确报"目标可能
位于可达边缘"。日志里 stage 消息会显示"端点通过 X 个，抓取预检通过 Y 个"。
