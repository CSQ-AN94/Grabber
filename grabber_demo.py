...
async def main_demo():
    ...
    try:
        ...
        await say("你好，我是Grabber，你的智能零售助手。我会查看货架上的商品，为您推荐和抓取商品，以及结算商品。请问您需要什么？")

        # --- Task A: 扫描与播报 ---
        # print("\n--- Task A: Inventory Scan ---")
        # print("Human: Grabber，货架上的商品更新了。")
        time.sleep(5)
        
        await say("我知道了。为了跟踪商品信息，让我来识别和播报货架上的所有商品。")
        arm.move_to_joints(app_config.arm.scanning_pose)
        time.sleep(3)
        await say("按货架从上到下，从左到右的顺序，第1层 奥利奥饼干, 橘子, 雀巢咖啡, 薯片；第2层：可口可乐, 农夫山泉矿泉水, 娃哈哈 AD钙奶, 红牛。")
        await say("货架上共有8样商品，请您任意选购，我会为您抓取。")

        # --- Task B: 图像理解与推荐 ---
        # print("\n--- Task B: Image-based Recommendation ---")
        # print("Human: 请进行根据我传入的图片进行商品推荐 (等待8秒)")
        time.sleep(8)
        ...
        await say("为您推荐选购商品为：农夫山泉矿泉水。Grabber开始抓取。")
        ...
        # print("Human: 我还想要奥利奥饼干 (等待4秒)")
        time.sleep(4)
        
        await say("好的，Grabber正在为您抓取奥利奥饼干")
        grasp(arm, "奥利奥饼干")
        await say("抓取完成，您还需要什么吗？")

        # print("\nHuman: 我还想要它正下方那个 (等待4秒)")
        time.sleep(4)
        
        await say("奥利奥饼干正下方的商品是可口可乐。Grabber正在为您抓取可口可乐。")
        grasp(arm, "可口可乐")
        await say("抓取完成，您还需要什么商品么？")

        # print("\nHuman: 有点困了，有什么推荐的？ (等待5秒)")
        time.sleep(5)

        await say("咖啡因可以缓解疲劳，这里有红牛和雀巢咖啡，都含有咖啡因，请问您要哪个呢？")
        
        # print("Human: 雀巢咖啡 (等待3秒)")
        time.sleep(3)
