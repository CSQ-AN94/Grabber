...
def test_release_confirmation_always_uses_independent_head_observer():
    target = np.array([0.02, 0.59, -0.135])
    ...
    np.testing.assert_allclose(localize_call[2], target)
...
def test_lift_confirmation_rejects_a_stationary_bottle_after_gripper_close():
    locked = _demo_with_head_adapter(np.zeros(3))[0].localize(
        "fixture", lambda: np.eye(4), DemoParams(), depth_prior_base=np.zeros(3)
    )
    locked.point_base = [0.0, 0.0, 0.5]
    demo, _calls = _demo_with_independent_head_measurement([0.0, 0.0, 0.5])

    with pytest.raises(SafetyAbort, match="抬升三维确认失败"):
        demo._confirm_lifted_target(locked)


def test_lift_confirmation_requires_fresh_depth_and_keeps_head_active():
    locked = Localization(
    ...
    demo, calls = _demo_with_independent_head_measurement([0.0, 0.0, 0.55])

    measured = demo._confirm_lifted_target(locked)

    np.testing.assert_allclose(measured.point_base, [0.0, 0.0, 0.55])
    assert demo.camera_name == "head"
    localize_call = next(call for call in calls if call[0] == "localize")
    np.testing.assert_allclose(localize_call[2], [0.0, 0.0, 0.55])
    assert localize_call[3] is False
